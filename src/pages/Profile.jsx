  ...
  useEffect(() => {
    const fetchUserData = async () => {
      ...
      try {
        ...
        const contributionsRes = await questionAPI.getMyQuestions();
        setUserContributions(contributionsRes.data);
        
        // If user is moderator, fetch moderator data
        if (user.role === 'moderator' || user.role === 'admin') {
          const moderatorRes = await userAPI.getModeratorDashboard();
          setModeratorData(moderatorRes.data);
        }
      } catch (error) {
                  ...
                  <>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                      <div className="bg-blue-50 dark:bg-blue-900 p-4 rounded-lg">
                        ...
                        <p className="text-2xl font-bold text-blue-600 dark:text-blue-300">{moderatorData.pendingCount}</p>
                      </div>
                      <div className="bg-green-50 dark:bg-green-900 p-4 rounded-lg">
                        <h3 className="font-semibold text-green-800 dark:text-green-200">Approved</h3>
                        <p className="text-2xl font-bold text-green-600 dark:text-green-300">{moderatorData.approvedCount}</p>
                      </div>
                      <div className="bg-red-50 dark:bg-red-900 p-4 rounded-lg">
                        <h3 className="font-semibold text-red-800 dark:text-red-200">Rejected</h3>
                        <p className="text-2xl font-bold text-red-600 dark:text-red-300">{moderatorData.rejectedCount}</p>
                      </div>
                    </div>
                    
                    <div className="space-y-4">
                      <h3 className="font-semibold text-lg">Recent Pending Questions</h3>
                      {moderatorData.recentPending.length === 0 ? (
                        <p className="text-gray-500">No pending questions</p>
                      ) : (
                        moderatorData.recentPending.map((question) => (
                          <div key={question._id} className="border rounded-lg p-4 bg-gray-50 dark:bg-gray-800">
                            <h4 className="font-medium line-clamp-2">{question.question}</h4>
                            <div className="flex items-center justify-between mt-2">
                              <span className="text-sm text-gray-500">
                                Submitted by {question.submittedBy?.username} • {new Date(question.createdAt).toLocaleDateString()}
                              </span>
